...
func (scw *JobFuncWrapper) Run(job *que.Job) error {
	for {
		...
		case ErrImmediateReschedule:
			scw.Logger.Printf("RESCHEDULE REQUESTED, RESTARTING... %s%s (%d)", job.Type, limitStringLength(string(job.Args), 30), job.ID)
			continue
		case ErrDidNotReschedule:
			scw.Logger.Printf("CRON JOB FINISHED AND HAS REQUESTED NOT TO BE RESCHEDULED %s%s (%d)", job.Type, limitStringLength(string(job.Args), 30), job.ID)
			return nil
		default:
			scw.Logger.Printf("FAILED WITH ERROR, RELY ON QUE TO RESCHEDULE %s%s (%d): %s", job.Type, limitStringLength(string(job.Args), 30), job.ID, err)
			return err
		}
	}
}
...
func (scw *JobFuncWrapper) tryRun(job *que.Job) error {
	scw.Logger.Printf("START %s%s (%d)", job.Type, limitStringLength(string(job.Args), 30), job.ID)
	defer scw.Logger.Printf("STOP %s%s (%d)", job.Type, limitStringLength(string(job.Args), 30), job.ID)
